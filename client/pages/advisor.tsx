...
const checkmarkSVG = (fill?: string) => {
	return (
		<svg
			className={`color-white bg-white ${fill ? fill : 'fill-blue-500'}`}
			focusable='false'
			aria-hidden='true'
			viewBox='0 0 24 24'
...
const Advisor: NextPage = () => {
	...
	const getQuestion = () => {
		...
		return (
			<div
				key={`question_${step.title}_${index}`}
				className={`mt-5 rounded shadow-lg shadow-slate-100 p-5 border ${index}`}
			>
				...
				{step.answers.map((answer, i) => (
					<div key={`question_${index}_answer_${i}`} className='flex items-center mt-2 mb-2'>
						<input
							onChange={() => answerSelected(index, i)}
							checked={i == step.selected}
							type='checkbox'
							value=''
							className='cursor-pointer w-5 h-5 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600'
						/>
						<label
							onClick={() => answerSelected(index, i)}
							htmlFor='checked-checkbox'
							className='ml-2 font-medium text-gray-900 dark:text-gray-300 text-base cursor-pointer'
						>
							{answer.answer}
						</label>
					</div>
